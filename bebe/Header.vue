  ...
  <div>
    <header class="app-header navbar">
      ...
      <div class="info" v-if="isLoginOK && isGroupOK">
        <p class="pic"><img src="/src/assets/images/logo.png" alt=""></p>
        <ul>
      ...
      <b-navbar-nav v-if="isLoginOK && isGroupOK" class="headerMenu">
        ...
        <b-nav-item v-if="commonIsLoginAuthGroupOK([5, 6])" class="item04" v-on:click="commonRouterLink('/front/noticeBoardList')">
          ...
          <b-badge v-if="getDataCountNotice > 0" pill variant="danger" style="left: 80% !important;">N</b-badge>
        </b-nav-item>
      </b-navbar-nav>
      <b-navbar-nav class="ml-auto headerRight">
        ...
        <!--
        <b-nav-item-dropdown v-if="$t('Lang.type') != 'ko-school'">
          <template slot="button-content">{{$t("Lang.name")}}</template>
          <b-dropdown-item v-for="item in $t('Lang.support')" v-on:click="changeLanguage(item.value)" :key="item.value"> {{item.text}}</b-dropdown-item>
        </b-nav-item-dropdown>
        -->
